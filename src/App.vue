	...
	<div id="allPages" class="big-box">
		<div class="border border-dark" style="height: 825px; width: 1350px">
			<transition name="fade" mode="out-in">
				<Intro v-if="this.currentFrame == 0" v-bind:images="this.images" v-on:complete="introExit" />
				<TryMe v-if="this.currentFrame == 1" v-bind:images="this.images" v-on:complete="tryMeExit" />
				<WhatIsThis
					v-if="this.currentFrame == 2"
				...
				/>
				<Outro v-if="this.currentFrame == 10" v-bind:images="this.images" v-on:reload="reload" />
			</transition>
		</div>
		<div class="container mt-3 mb-2">
			<div class="row">
				<div class="col small p-0">
					Created by <a href="https://www.scify.gr/site/en" target="_blank">SciFY</a> and
...
export default {
	components: {
		...
		TryMe: () => import("./components/TryMe.vue"),
		WhatIsThis: () => import("./components/WhatIsThis.vue"),
		...
		fetchNextInstance() {
			if (this.allData.length > 0) {
				...
				if (currentElement !== null) {
					...
					if (indexOfCurrentElement < this.allData.length - 1) newElementIndex = indexOfCurrentElement + 1;
				}
